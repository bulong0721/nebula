...
public class ClassPathMQScanner extends ClassPathBeanDefinitionScanner {
    ...
    public void registerFilters() {
        ...
        if (produceOn) {
            addIncludeFilter(new CombineTypeFilter(Producer.class, MessageSender.class));
            acceptAllInterfaces = false;
        }
        if (consumeOn) {
            addIncludeFilter(new CombineTypeFilter(Consumer.class, MessageListener.class));
            acceptAllInterfaces = false;
        }
    ...
    private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
        ...
        for (BeanDefinitionHolder holder : beanDefinitions) {
            ...
            if (metadata.hasAnnotation(Producer.class.getName())) {
                ...
                ProduceConfig producerConfig = toProducerConfig(metadata.getAnnotationAttributes(Producer.class.getName()));
                ...
                definition.getPropertyValues().add("produceConfig", producerConfig);
                definition.setAutowireMode(AbstractBeanDefinition.AUTOWIRE_BY_TYPE);
            } else if (metadata.hasAnnotation(Consumer.class.getName())) {
                ConsumeConfig consumerConfig = toConsumerConfig(metadata.getAnnotationAttributes(Consumer.class.getName()));
                definition.getPropertyValues().add("config", consumerConfig);
            }
        }
    }

    private ProduceConfig toProducerConfig(Map<String, Object> metadataSet) {
        AnnotationAttributes attributes = AnnotationAttributes.fromMap(metadataSet);
        ProduceConfig config = new ProduceConfig();
    ...
    private ConsumeConfig toConsumerConfig(Map<String, Object> metadataSet) {
        AnnotationAttributes attributes = AnnotationAttributes.fromMap(metadataSet);
        ConsumeConfig config = new ConsumeConfig();
        config.setThreadPool(attributes.getString("threadPool"));
